...
for (const [persona, data] of Object.entries(mapping)) {
  [...data.core, ...data.sat].forEach(code => {
    ...
    if (name.includes('貨幣市場') || name.includes('R/') || name.includes('RE/') || name.includes('TISA') || name.includes('R級別') || name.includes('R USD') || name.includes('R EUR') || name.includes('R GBP') || name.includes('R AUD')) {
...
const availableReplacements = mockFunds.filter(f => {
  const name = f.name;
  if (name.includes('貨幣市場') || name.includes('R/') || name.includes('RE/') || name.includes('TISA') || name.includes('R級別') || name.includes('R USD') || name.includes('R EUR') || name.includes('R GBP') || name.includes('R AUD')) return false;
  // if (allUsedFunds.has(f.code)) return false;
  return true;
});

console.log('Available replacements count:', availableReplacements.length);
console.log('Available replacements:', availableReplacements.map(f => ({ code: f.code, name: f.name, risk: f.risk })));
