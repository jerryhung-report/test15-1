...
const fundMap = {};
mockFunds.forEach(f => {
  // If duplicate, keep the one with longer name (usually the one from CSV)
...
const availableFunds = Object.values(fundMap).filter(f => !isExcluded(f) && !allUsedFunds.has(f.code));
